...
def get_model_path(model: str, update_model: bool = False):
    ...
    # If a vendored local model exists under the repository's `models/` directory, prefer it
    try:
        repo_models_dir = (BASE_DIR / "models").resolve()
    except Exception:
        repo_models_dir = None

    if repo_models_dir:
        # normalize model name to folder name (e.g. sentence-transformers/all-MiniLM-L6-v2 -> all-MiniLM-L6-v2)
        model_name = model.split("/")[-1]
        vendored_path = repo_models_dir / model_name
        if vendored_path.exists():
            log.info(f"Using vendored model at: {vendored_path}")
